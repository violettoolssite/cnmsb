...
### 智能上下文感应补全（AI 级别）
基于自然语言理解，自动识别你的意图并查找系统路径。操，这功能太他妈智能了：
...
**特性（都他妈很实用）：**
- **自然语言理解**：理解 `export JAVA_HOME=` 的意图，自动查找 Java 路径，不用你他妈手动找
- **自动路径查找**：在 `/opt`、`/usr/local`、`/usr/lib/jvm` 等常见位置查找，省得你到处翻
- **智能建议**：根据已定义的 `*_HOME` 变量，自动生成 PATH 配置建议，不用一个个敲
- **大小写智能匹配**：输入 `$path` 自动建议 `$PATH`，保持变量名一致性，不会搞错
